...
#' @param ROImap1 Hash map with tumour ROI names as keys and tumour ROI coordinates as values.
#' @param ROImap2 Hash map with T cell ROI names as keys and T cell ROI coordinates as values.
#' @param ROImapField1 Hash map with tumour ROI names as keys and assignment to grid as value.
...
#'
#' @examples
compute_distTimepoint_withoutGrid <- function( timepoint,
                                         gridWidth = 100,
  ...
  pairs <- compute_pairs_timepoint( timepoint, ROImap1 = tumourROImap, ROImap2 = tcellROImap)
  ...
  cell_distances <- sapply( pairs_modified, compute_cellDistance_withoutGrid,
                            gridWidth = 100,
                            ROImap1 = tumourROImap,
                            ROImap2 = tcellROImap,
                            ROImapField1 = tumourROImap_NamesToGrid,
...
compute_pairs_timepoint <- function( time, ROImap1 = tumourROImap, ROImap2 = tcellROImap ){
  
  # Cast time point to character
  time <- as.character( time )
  
  # Get Track names for this time point
  tumourROIs_timepoint <- ROImap1[[ time ]]
  tcellROIs_timepoint <- ROImap2[[ time ]]
  
  # Check whether there are any tumour and T cells for this time point
  if( any( tumourROIs_timepoint ==  "No value for this key" ) || any( tcellROIs_timepoint == "No value for this key")){
    pairs <- list()
  } else{
    # Compute all possible pairs for this time point
    pairs <- expand.grid( tumourROIs_timepoint, tcellROIs_timepoint )
  }
...
compute_cellDistance_withoutGrid <- function( pair,
                                        gridWidth = 100,
                                        ROImap1 = tumourROImap_center,
                                        ROImap2 = tcellROImap_center,
                                        ROImapField1 = tumourROImap_NamesToGrid,
                                        ROImapField2 = tcellROImap_NamesToGrid){
  
  # Get names
  tumourName <- as.character( pair[[1]] )
  tcellName <- as.character( pair[[2]] )
  
  # Check distance of center points
  
  tumourField <- ROImapField1[[ tumourName ]]
  tcellField <- ROImapField2[[ tcellName ]]
  
  abs_dist_fields <- abs( tumourField - tcellField )
  ...
  # } else{
    ...
    # Get coordinates
    tumourCoords <- tumourROImap[[ tumourName ]]
    tcellCoords <- tcellROImap[[ tcellName ]]
    
    # Transform coordinates to polygon
    tumourPolygon <- getPolygon( tumourCoords )
    tcellPOlygon <- getPolygon( tcellCoords )
    
    # Compute distance
    dist <- st_distance(x = tumourPolygon, y= tcellPOlygon, by_element = TRUE)
  # }
